...
const lista = document.getElementById("emails");
...
fetch('01.xml')
    ...
    .then(data => {
        ...
        const mail = data.getElementsByTagName('remitente');
        console.log(mail);
        // Recorrer cada libro y mostrar su información
        
        const remi = mail.getElementsByTagName('remitente').textContent;
        //acceder al atributo id
        const dest = mail.getElementsByTagName('des').getElementsByTagName('remitente').textContent;
        // acceder al genero del libro
        const genero = mail.getAttribute('genero');    
        const autor = mail.getElementsByTagName('autor')[0].textContent;

        // Crear un LI para mostrar la información
        const li = document.createElement('li');
        li.textContent = ` remi: ${remi} - desti: ${dest} - ${genero} -Primer Autor: ${autor}`;
